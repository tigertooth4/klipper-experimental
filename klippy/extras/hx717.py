...
class HX717(load_cell.LoadCellSensor):
    def __init__(self, config):
        ...
        sclk_pin_params = ppins.lookup_pin(config.get('sck_pin'))
        self.dout_pin = dout_pin_params['pin']
        self.sclk_pin = sclk_pin_params['pin']
        self.mcu = mcu = dout_pin_params['chip']
        # HX717 sps choices
        self.sps = config.getchoice('sps', {320: 320, 80: 80, 20: 20, 10: 10}
                                    , default=320)
        # HX717 gain/channel choices
        self.gain_channel = config.getchoice('gain', {'A-128':1, 'B-64': 2
                                    , 'A-64': 3, 'B-8': 4}, default='A-128')
        ppins.register_chip(self.name, self)
        mcu.register_config_callback(self._build_config)
    def _build_config(self):
        self.mcu.add_config_cmd("config_hx71x oid=%c dout_pin=%d" \
                                    " sclk_pin=%d gain_channel=%d"
            % (self.oid, self.dout_pin, self.dout_pin, self.sclk_pin
               , self.gain_channel))
    def get_oid(self):
        return self.oid
